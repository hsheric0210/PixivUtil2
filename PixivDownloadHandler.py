    ...
    while retry_count <= max_retry:
        ...
        try:
            try:
                ...
                if PixivHelper.aria2_inputfile is None:
                ...
                if image is not None:
                    ...
                    if PixivHelper.aria2_inputfile is None and db_filename is not None and os.path.isfile(db_filename):
...
def perform_download(url, file_size, filename, overwrite, config, referer=None, notifier=None):
    ...
    (downloadedSize, filename) = PixivHelper.download_image(url, filename, res, file_size, overwrite, referer)
    res.close()
    gc.collect()
